...
def integral_analytique(a: float, b: float, p1: float, p2: float, p3: float, p4: float):
    """Intégrale analytique de f entre a et b"""
    term1 = p1 * (b - a)
    term2 = p2 * (b ** 2 - a ** 2) / 2.0
    term3 = p3 * (b ** 3 - a ** 3) / 3.0
    term4 = p4 * (b ** 4 - a ** 4) / 4.0
    return term1 + term2 + term3 + term4


def integral_rectangle(a: float, b: float, p1: float, p2: float, p3: float, p4: float, n: int = 10):
    """Intégration numérique par la méthode des rectangles sur n segments (version classique)."""
